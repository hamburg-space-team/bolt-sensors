...
namespace Sensor {
    ...
    /// Single error code vocabulary for the library. Driver and bus calls
    /// return `std::expected<T, Error>`; on failure the caller gets
    ...
    /// Hardware context payload detailing the specific state or data involved in a failed operation.
    /// This acts as a tagged union where the active field is strictly determined
    /// by the accompanying ErrorCode.
    ///
    /// @ingroup utils
    union ErrorContext {
        uint32_t raw;              ///< use instead of interpreting the error in runtime
        uint8_t register_address;  ///< the specific hardware register targeted when the fault occured
        uint8_t device_address;    ///< the target address of the unresponsive or failed sensor
        uint16_t device_id;        ///< the unidentified WHO_AM_I / hardware ID from the device
        uint32_t bus_status_flags; ///< a status register dump for low-level electrical debugging

        [[nodiscard]] static constexpr ErrorContext from_register(uint8_t reg) noexcept {
            ErrorContext context{0U};
            // NOLINTNEXTLINE(cppcoreguidelines-pro-type-union-access)
            context.register_address = reg;

            return context;
        }

        [[nodiscard]] static constexpr ErrorContext from_device(uint8_t addr) noexcept {
            ErrorContext context{0U};
            // NOLINTNEXTLINE(cppcoreguidelines-pro-type-union-access)
            context.device_address = addr;

            return context;
        }

        [[nodiscard]] static constexpr ErrorContext from_id(uint16_t id) noexcept {
            ErrorContext context{0U};
            // NOLINTNEXTLINE(cppcoreguidelines-pro-type-union-access)
        ...
        [[nodiscard]] static constexpr ErrorContext from_bus_state(uint32_t flags) noexcept {
            ErrorContext context{0U};
            // NOLINTNEXTLINE(cppcoreguidelines-pro-type-union-access)
            context.bus_status_flags = flags;
    ...
    struct ErrorClock {
        ...
        static inline ClockFn now_us = nullptr;
    };
    ...
    struct Error {
        ...
        ErrorContext context{0U};              ///< hardware payload (register, ID, or raw flags)
        ...
        uint8_t reserved = 0U;                 ///< explicit padding to get to 20 bytes
    ...
    static_assert(sizeof(Error) <= 20U, "Error rides in every Result<T> - keep it small");
    ...
    [[nodiscard]] inline Error make_error(ErrorCode code, Step origin, uint16_t line, ErrorContext context) noexcept {
        Error e{};
        e.code = code;
        e.context = context;
        e.line = line;
    ...
    /// Failure origin where a specific source line is not applicable
    [[nodiscard]] inline std::unexpected<Error> fail(ErrorCode code, Step origin) noexcept {
        return std::unexpected(make_error(code, origin, 0U, ErrorContext{0U}));
    }
    ...
    [[nodiscard]] inline std::unexpected<Error> fail(ErrorCode code, Step origin, uint16_t line) noexcept {
        return std::unexpected(make_error(code, origin, line, ErrorContext{0U}));
    }

    /// Overloaded origin of a failure that includes a specific hardware context payload.
    ///
    ///   return fail(ErrorCode::NACK_RECEIVED, Step::I2C_WRITE, __LINE__, ErrorContext::from_device(0x4A));
    [[nodiscard]] inline std::unexpected<Error> fail(ErrorCode code, Step origin, uint16_t line,
                                                     ErrorContext context) noexcept {
        return std::unexpected(make_error(code, origin, line, context));
    }
